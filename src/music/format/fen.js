...
//
export function fen_music(fen) {
    let staff = fen.split('\n');
    if (staff[0] === 'brace') {
        let s1 = fen_staff(staff[1]), s2 = fen_staff(staff[2]);
        if (s1 && s2) {
            return [s1, s2];
        }
    }
...
export function read_time(time) {
    let [_nb_note_value, _note_value] = time.split('/');
    let nb_note_value = uci_nb_note_value(parseInt(_nb_note_value)), note_value = uci_note_value(parseInt(_note_value));
    if (nb_note_value && note_value) {
        return make_time_signature(nb_note_value, note_value);
    }
}
export function read_notation(notation) {
    let n = notation[0];
